package com.ahmet.eldrctestapplication.virtual_joystick;
...
public class JoystickView extends View
    ...
    */
    ...
    public double[] getJoystickAxesDouble() {
        // Convert the angle from degrees to radians
        double angleRadians = Math.toRadians(getAngle());

        // Calculate polar coordinates
        double polarX = getStrength() * Math.cos(angleRadians);
        double polarY = getStrength() * Math.sin(angleRadians);

        // Normalize the polar coordinates to a range of -100 to 100
        double maxX = 100.0;
        double minX = -100.0;
        double maxY = 100.0;
        double minY = -100.0;

        double normalizedX = ((polarX + 1) / 2) * (maxX - minX) + minX;
        double normalizedY = ((polarY + 1) / 2) * (maxY - minY) + minY;

        // Create an array to store the normalized X and Y coordinates
        double[] normalizedCoordinates = {normalizedX, normalizedY};

        return normalizedCoordinates;
    }


    public int[] getJoystickAxes() {
        // Convert the angle from degrees to radians
        double angleRadians = Math.toRadians(getAngle());
        ...
        double normalizedX = (((polarX + 1) / 2) * (maxX - minX) + minX) / maxX;
        double normalizedY = (((polarY + 1) / 2) * (maxY - minY) + minY) / maxY;

        // Create an array to store the normalized X and Y coordinates
        int[] normalizedCoordinates = {(int) normalizedX, (int) normalizedY};
